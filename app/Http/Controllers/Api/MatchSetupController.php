...
class MatchSetupController extends Controller
{
    ...
    {
        try {
            $mode = $request->input('mode', 'user'); // default: user
            ...
            if ($mode === 'admin') {
                ...
                return response()->json([
                    'message' => 'Masuk sebagai administrator',
            ...
            $query = MatchPersonnelAssignment::where('tournament_name', $data['tournament_name'])
                ...
                ->where('tipe_pertandingan', $data['match_type']);
   ...
   public function store_22(Request $request)
    {
        try {
            $data = $request->validate([
                'tournament_name' => 'required|string',
