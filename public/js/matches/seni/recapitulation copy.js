$(document).ready(function () {
    ...
    globalChannel.bind('seni.match.changed', function (data) {
        ...
        window.location.href = url + `/matches/seni/${data.new_match_id}/recap`; // Sesuaikan path kalau perlu
    });
    ...
    function fetchMatchData() {
        $(".loader-bar").show();
         $.get(`${url}/api/local-matches/seni/${matchId}`, function (data) {
            ...
            $("#tournament-name").text(data.tournament_name).css("font-size", "17px");

            /*if (data.match_type === 'seni_ganda' || data.match_type === 'solo_kreatif') {
                $("#unsur-tunggal-ganda").hide();
                $("#unsur-regu-solo").show();
            } else {
                $("#unsur-tunggal-ganda").show();
