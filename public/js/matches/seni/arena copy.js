...
function fmtTime(input, mode = 'auto') {
  // --- normalisasi ke detik ---
  const toSeconds = (v) => {
    ...
    if (m) {
      ...
      const ss = Math.max(0, Math.min(59, parseInt(m[2], 10)));
      return mm * 60 + ss;
    }

    // menit desimal (ada koma/titik)
    if (/[.,]/.test(raw)) {
      const mins = parseFloat(raw.replace(',', '.'));
      return isNaN(mins) ? 0 : Math.max(0, Math.round(mins * 60));
    }

    // digit-only → anggap DETIK
  ...
  // default 'auto' & 'm,dd' → menit desimal (2 desimal, pakai koma)
  const minutesDecimal = mm + ss / 60;
  return minutesDecimal.toFixed(2).replace('.', ',');
}

// Detik (integer) → "mm:ss" atau "m,dd" (pakai koma, 2 desimal)
function fmtSeconds(sec, mode = 'm,dd', digits = 2) {

  let s = Number(sec);
  if (!Number.isFinite(s)) s = 0;
  s = Math.max(0, Math.round(s)); // jaga-jaga kalau bukan integer
  ...
  // default: "m,dd" → menit desimal dengan koma
  const minutes = s / 60;
  return new Intl.NumberFormat('id-ID', {
    minimumFractionDigits: digits,
    maximumFractionDigits: digits,
  }).format(minutes); // contoh: 181 → "3,02", 185 → "3,08"
}
...
function renderWinnerModal(data) {
  ...
  const name        = (data?.winner_name || '-').toString().trim();
  const cont        = (data?.contingent  || '-').toString().trim();
  const corner      = (data?.corner      || '').toLowerCase();
  const reasonLabel = (data?.reason_label|| '').toString();

  // --- size logic: kecilin kalau ada '&' atau ',' (ganda/regu) ---
  const parts = name.split(/[&,]/).map(s => s.trim()).filter(Boolean);
  const isMulti = parts.length >= 2;

  // Reset kelas fs-* dulu biar gak numpuk
  nameEl.classList.remove('fs-1','fs-2','fs-3','fs-4','fs-5','winner-compact','winner-tiny');

  // Default besar untuk tunggal
  let sizeClass = 'fs-1'; // Bootstrap 5: fs-1 paling besar
  if (isMulti) {
    // 2 nama → agak kecil; 3+ atau nama panjang → lebih kecil
    if (parts.length >= 3 || name.length > 40) {
      sizeClass = 'fs-3';          // lebih kecil
      nameEl.classList.add('winner-compact');
    } else {
      sizeClass = 'fs-2';          // sedikit kecil
      nameEl.classList.add('winner-compact');
    }
  }
  nameEl.classList.add(sizeClass);
  ...
  // (opsional) badge warna sudut
  badgeEl.classList.remove('bg-primary','bg-danger','d-none');
  if (corner === 'blue')      badgeEl.classList.add('bg-primary');
  else if (corner === 'red')  badgeEl.classList.add('bg-danger');
  else                        badgeEl.classList.add('d-none');
}


function renderWinnerModal__(data) {
